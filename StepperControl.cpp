...
void StepperControl::loadSettings()
{
  ...
  encoderZ.loadMdlEncoderId(_MDL_Z);
    ...
    encoderX.loadCANbusEncoderId(ENC_X2_CAN_ID);
    encoderX.loadMicrosteppingValue(X2_MICROSTEP);
    encoderY.loadCANbusEncoderId(ENC_Y_CAN_ID);
    encoderY.loadMicrosteppingValue(Y_MICROSTEP);
    encoderZ.loadCANbusEncoderId(ENC_Z_CAN_ID);
    encoderZ.loadMicrosteppingValue(Z_MICROSTEP);
  #endif
                                 ...
                                 bool xHome, bool yHome, bool zHome)
{
  ...
  axisActive[2] = true;
    ...
    CANbusFunctions::getInstance()->CANaxisActive = true;
  #endif
  ...
  if (xHome || yHome || zHome)
  {
    ...
    axisActive[2] = zHome;
      ...
      {
        CANbusFunctions::getInstance()->CANaxisActive = false;
      }
    #endif
  }
...
void StepperControl::checkEncoders()
{
  ...
  */
  ...
  // Injected interrupt code for CANbus based encoders
    ...
    // Move new received message(s) into proper container (a single character, 32bit number or a combination are the only options currently)
    ...
    // Time to request another update from encoders?
